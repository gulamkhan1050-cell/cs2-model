"""
cs_trainer.py  -  Counter-Strike 2 pro-match predictor (run on your PC)

Data: PandaScore free plan (past matches, map winners and round scores, tournament rosters, tiers).
      Sign up at pandascore.co, copy your token, paste it below or set PANDASCORE_TOKEN.

Setup:   pip install requests numpy
Run:     python cs_trainer.py                 # ~12 months
         python cs_trainer.py --months 18
         python cs_trainer.py --demo          # synthetic data, offline pipeline check

Raw data is cached in cache_cs/ so re-runs only download what is new.
"""
...
# ------------------------------------------------------------------ config
TOKEN = os.environ.get("PANDASCORE_TOKEN", "")   # <-- or paste your PandaScore token here: "abc123..."
BASE = 1500.0
K_TEAM = 32.0
K_PLAYER = 20.0
...
class RateLimited(Exception):
    """PandaScore's hourly or monthly quota is spent. Not an error we should crash on:
    whatever is already cached is still usable, and the next run picks up where this one stopped."""


def ps_get(path, params=None, tries=4, soft=False):
    if not TOKEN:
        sys.exit("No PandaScore token. Paste it into TOKEN at the top of cs_trainer.py.")
    for i in range(tries):
        try:
            r = requests.get(f"{API}{path}", params=params,
                             headers={"Authorization": f"Bearer {TOKEN}", "Accept": "application/json"}, timeout=60)
        except Exception as e:
            print(f"  network error ({e}), retrying"); time.sleep(10 * (i + 1)); continue
        if r.status_code == 429:
            wait = int(r.headers.get("Retry-After") or 0) or 60 * (i + 1)
            if soft and i >= 1:
                raise RateLimited(path)
            print(f"  rate limited, waiting {wait}s"); time.sleep(min(wait, 300)); continue
...
def fetch_past_matches(months):
    ...
    while True:
        try:
            rows = ps_get("/csgo/matches/past", {
                "sort": "-begin_at", "per_page": 100, "page": page,
                "range[begin_at]": f"{lo},{hi}", "filter[status]": "finished",
            }, soft=True)
        except RateLimited:
            json.dump(sorted(have.values(), key=lambda m: (m["begin_at"], m["id"])), open(f, "w", encoding="utf-8"))
            print(f"  PandaScore quota spent at page {page}. Keeping the {len(have)} matches fetched so far;")
            print("  run again later and it will carry on from here.")
            break
        if not rows:
            break
        for m in rows:
        ...
        if page % 5 == 0:                                    # checkpoint, so a quota stop never loses work
            json.dump(sorted(have.values(), key=lambda m: (m["begin_at"], m["id"])), open(f, "w", encoding="utf-8"))
        if len(rows) < 100:
            break
        page += 1
        time.sleep(1.2)   # free plan quota is easy to exhaust; go gently
...
def slim(m):
    ...
    for g in m.get("games") or []:
        ...
        if g.get("finished") and w.get("id") and not g.get("forfeit"):
            rs = {str(r.get("team_id")): r.get("score") for r in (g.get("rounds_score") or []) if r.get("team_id") is not None}
            maps.append({"pos": g.get("position", 0), "winner": w["id"], "rounds": rs})
...
def fetch_upcoming(days=7, back=1):
    """Matches from `back` days ago to `days` days ahead (PandaScore /csgo/matches), slimmed.
    ...
    while True:
        try:
            rows = ps_get("/csgo/matches", {"sort": "begin_at", "per_page": 100, "page": page, "range[begin_at]": f"{lo},{hi}"}, soft=True) or []
        except RateLimited:
            print("  quota spent while reading the schedule; using what we have")
            break
        for m in rows:
...
def fetch_rosters(tournament_ids):
    ...
    for i, tid in enumerate(todo):
        try:
            res = ps_get(f"/tournaments/{tid}/rosters", soft=True)
        except RateLimited:
            print(f"  quota spent after {i} rosters; keeping those and carrying on (next run continues)")
            stopped = True; break
        except SystemExit:
            raise
        except Exception as e:
            print(f"  roster {tid} failed: {e}"); have[str(tid)] = {}; time.sleep(0.7); continue
        try:
            ...
            print(f"  roster {tid} unreadable: {e}")
            have[str(tid)] = {}
        if (i + 1) % 25 == 0:
        ...
        time.sleep(1.0)
    json.dump(have, open(f, "w", encoding="utf-8")); json.dump({str(k): v for k, v in PNAMES.items()}, open(pf, "w", encoding="utf-8"))
    if stopped: print(f"  rosters: {len(have)} cached in total so far")
    return have
...
def demo_matches(n=2500, teams=40):
    ...
    for i in range(n):
        ...
        while max(wins) < (bo + 1) // 2:
            ...
            lose = random.randint(3, 11) if random.random() < 0.85 else random.randint(12, 13)
            wins[0 if w == a else 1] += 1; maps.append({"pos": len(maps) + 1, "winner": w, "rounds": {str(w): 13 if lose < 12 else lose + 2, str(a if w == b else b): lose}})
        ts = now - (n - i) * 3600 * 3
...
class State:
    ...
    def update_map(self, a, b, pa_, pb_, a_won, ts, tier=3, rounds=None):
        win = 1.0 if a_won else 0.0
        e = expected(self.team_elo[a], self.team_elo[b])
        mov = 1.0
        if rounds and str(a) in rounds and str(b) in rounds and rounds[str(a)] is not None and rounds[str(b)] is not None:
            diff = abs(int(rounds[str(a)]) - int(rounds[str(b)]))                   # 13-3 says more than 13-11
            mov = 0.6 + 0.16 * math.log1p(min(diff, 13))
...
def main():
    ...
    market = [] if args.demo else load_market(args.odds, "cs2")
    ...
    if not args.demo and os.path.exists(args.odds):
        try:
            ...
            for k, r in raw.items():
                if r.get("game") != "cs2" or r.get("closing") or not r.get("series"): continue
                ser = r["series"]
    ...
    for m in matches:
        ...
        for g in m["games"]:
            ...
            st.update_map(a, b, PA, PB, g["winner"] == a, m["ts"], m["tier"], g.get("rounds"))
        st.after_match(m, pa, pb)
    X, y, MK = np.array(X), np.array(y), np.array(MK)
    nr = sum(1 for m in matches for g in m["games"] if g.get("rounds"))
    print(f"\n{len(matches)} matches, {len(X)} games ({nr} with round scores for margin-of-victory Elo), {no_roster} matches without roster data")
    ...
    def roster_report(t, expected, now):
        ...
        return {"expected": [pinfo(p) for p in exp], "in": [pinfo(p) for p in ins], "out": [pinfo(p) for p in outs], "known": bool(expected),
                "core4_maps": core, "source": "tournament roster" if expected else "last five seen playing",
                "maps_together": L["games"], "days_together": round((now - L["first"]) / 86400, 1) if L["first"] else 0, "best_lineup_maps": best,
    ...
    out = {"game": "cs2", "as_of": datetime.now(timezone.utc).strftime("%Y-%m-%d"), "matches_used": len(matches), "games_used": int(len(X)),
