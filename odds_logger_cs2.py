"""
odds_logger_cs2.py  -  CS2 market prices, no API key needed (Polymarket public API).

Polymarket prices tier-1 CS2 matches (Majors, IEM, BLAST, ESL, PGL...). It is a prediction market,
...
Run:   python odds_logger_cs2.py --backfill 180    # once: history for the last 180 days
       python odds_logger_cs2.py                   # one pass over upcoming matches
       python odds_logger_cs2.py --loop            # keep running, one pass every 30 minutes
       python odds_logger_cs2.py --loop --every 15 # ... or every 15 minutes
...
Output: odds_log.json next to cs_trainer.py (picked up automatically).
"""
...
OUT = "odds_log.json"
UA = {"User-Agent": "CS2MatchLogger/1.0"}
CS_WORDS = re.compile(r"counter[- ]?strike|\bcs2\b|\bcs:?go\b|blast|\biem\b|esl pro league", re.I)
...
# ------------------------------------------------------------- find the CS2 tag(s)
def cs2_tags():
    """Polymarket tag ids whose label/slug mention Counter-Strike 2 / CS2."""
    ids = set()
    try:
        for s in get(f"{GAMMA}/sports") or []:
            blob = json.dumps(s).lower()
            if CS_WORDS.search(blob):
    ...
    if not ids:
        try:
            ...
            while True:
                ...
                for t in tags:
                    if CS_WORDS.search((t.get("label") or "") + " " + (t.get("slug") or "")): ids.add(str(t["id"]))
...
def record(log, key, ev, teamA, teamB, start, p_first, p_last, now, extra=None):
    ...
    rec = log.get(key) or {"game": "cs2", "league": ev.get("title") or ev.get("slug") or "", "home": teamA, "away": teamB, "date": start,
...
def backfill(log, days):
    ...
    tags = cs2_tags(); print(f"  CS2 tag ids on Polymarket: {tags or 'none found'}")
    done = 0; seen = set()
    for tag in tags:
...
def oddspapi_pass(log, backfill_days=0):
    ...
    try:
        ...
        for f in fx[:60]:
            ...
            if got:
                log[key] = {"game": "cs2", "league": f.get("tournamentName", ""), "home": f.get("participant1Name"), "away": f.get("participant2Name"), "date": f.get("startTime"),
